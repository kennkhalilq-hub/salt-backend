...
async def telegram_webhook(request: Request):
    ...
    if "callback_query" in data:
        ...
        if not user:
            conn.close()
            await answer_callback(cq_id, f"❌ User '{username}' not found.")
            await send_telegram(ADMIN_CHAT_ID, f"❌ Could not find user: <b>{username}</b>
All users in DB: {', '.join(all_names) if all_names else 'none'}")
            return {"ok": True}
