...
API_URL        = "https://salt-wormgpt.wasmer.app/index.php"   # ✏️ fill this in
API_MODEL      = "salt-2.0"     # ✏️ fill this in
# ================================================================
...
async def telegram_webhook(request: Request):
    ...
    if "callback_query" in data:
        ...
        # Compare as strings, strip whitespace just in case
        if chat_id.strip() != str(ADMIN_CHAT_ID).strip():
            await answer_callback(cq_id, "⛔ Not authorized.")
            return {"ok": True}

        # callback_data format is "action:username"
        if ":" not in cq_data:
            await answer_callback(cq_id, "❌ Invalid action.")
            return {"ok": True}
        ...
        # Debug: list all users if user not found
        ...
        if not user:
            ...
            names_list = ", ".join(all_names) if all_names else "none"
            await send_telegram(ADMIN_CHAT_ID, "❌ Could not find user: <b>" + username + "</b>\nAll users in DB: " + names_list)
            return {"ok": True}
